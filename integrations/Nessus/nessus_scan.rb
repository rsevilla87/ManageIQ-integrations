...
# Get provisining object
$prov = $evm.root["miq_request"] || $evm.root["service_template_provision_task"]
...
def error_handler(err)
  ...
  log(:error, "Nessus scan error: #{err}]")
  $evm.set_state_var("reason", "Nessus scan error: #{err}")
  exit MIQ_STOP
end
...
def check_availability
  ...
  begin
    ...
    error_handler("error", "Error accessing to #{$protocol}://#{$serverurl}:#{$port}/\n#{err}")
  end
end
...
def get_scan_id(scanname)
  ...
  while true
    ...
    if not found
      error_handler("Scan #{scanname} not found in Nessus")
    end
  end
end
